...
%type <intval> ifstmt ifprefix elseprefix
%type <intval> whilestmt whilecond whilestart
%type <intval>  M N
%type <statement> loopstmt stmt break continue
...
assignexpr:   lvalue {
                            ...
                            if(local == 1){
                                ...
                                if(LookupForLval($1->strConst, scope)==0) {
                                    ...
                                    else if(LookupForLval($1->strConst,0)==1){ //auth peira3aaaaa
                                        $1->sym=findEntry2($1->strConst,0);                                       
                                    }
                                    else if(LookupForLval($1->strConst,0)==2){
                                         printf(RED"ERROR"RESET" in line %d, %s is a user func, invalid use\n",yylineno,$1->strConst);
                                    }
