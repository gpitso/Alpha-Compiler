...
struct backpatch* truelist=NULL;
struct backpatch* falselist=NULL;
...
backpatch* make_tlist(unsigned int label) {  //gia continue kai break
	backpatch* head=truelist;
...
backpatch* make_flist(unsigned int label) {  //gia continue kai break
	backpatch* head=falselist;
	backpatch* newn=(backpatch*)malloc(sizeof(backpatch));
	newn->quadadd=label;	
	newn->next=NULL;
...
void patchlabel(unsigned quadNo, unsigned label) {
	printf("patch : q %d l %d\n",quadNo,label);
    assert(quadNo < currQuad);
    quads[quadNo].label = label;
}

void backpatchh(backpatch* list , unsigned int quadno){
    backpatch* tmp = list;
    while(tmp != NULL){
        patchlabel(tmp->quadadd, quadno);
        tmp = tmp->next;
    }
    return ;
